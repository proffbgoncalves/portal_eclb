...
abstract class AbstractNotablePersonDataMapper implements NotablePersonDataMapper {
  ...
  List generateCountStatement() {
    List statement = ["SELECT COUNT(patrimonyPersonId) FROM NOTABLEPERSONS"];
    return statement;
  }
  ...
  List generateDeleteStatement(Object id) {
    ...
    List statement = ["DELETE FROM NOTABLEPERSONS WHERE PATRIMONYPERSONID = ?", [id]];
    return statement;
  }
  ...
  List generateFindByPatrimonyPersonId(Object id) {
    throwIf(!(id is int), new Exception("Id parameter is not an instance of int."));
    List statement = ["SELECT * FROM NOTABLEPERSONS WHERE PATRIMONYPERSONID = ?", [id]];
    return statement;
  }
  @override
  ...
  List generateInsertStatement(Object dto) {
    ...
    List statement = [
      "INSERT INTO NOTABLEPERSONS (PATRIMONYPERSONID) VALUES (?)",
      [
  ...
  List generateFindAllStatement([int limit = 0, int offset = 0]) {
    if (limit > 0 && offset >= 0) {
      return ["SELECT * FROM NOTABLEPERSONS LIMIT = ? OFFSET = ?", [limit, offset]];
    } else if (limit == 0 && offset == 0) {
      return ["SELECT * FROM NOTABLEPERSONS"];
    } else {
